# Phase 1 — 차원 가변 유체 라우팅 프로토타입

> 이 문서는 Phase 1 당시의 구현 사양을 보존한 기록이다. 현재 Phase 2에서는
> STREAM과 POOL을 별도 입자 솔버로 유지하지 않고 하나의 2D SWE 수심·유속장으로
> 계산한다. 절벽 flux만 3D 입자로 전환하며, GNN은 SPLASH 영역의 잔차 Δv만
> 예측한다. 최신 전체 설계와 진행 상태는 `전체.md`를 기준으로 한다.
...
- 스크린 공간 렌더링 (메타볼, 프레넬, 굴절 등) — Phase 4
- 엔진(Unreal) 통합 — Phase 4
